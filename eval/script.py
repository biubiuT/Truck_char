...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    for k in subm:
        if (k in gt) == False:
            raise Exception("The sample %s not present in GT" % k)#zip -r myfile.zip ./* 
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    Rectangle = namedtuple('Rectangle', 'xmin ymin xmax ymax')#具名元组，不必再通过索引值进行访问，可以把它看做一个字典通过名字进行访问，只不过其中的值是不能改变的，第一个元素为元组名，第二个元素为元组内具有的属性
    ...
    for resFile in gt:
        ...
        gtFile = rrc_evaluation_funcs.decode_utf8(gt[resFile])#返回unicode编码，保证标签数据不会乱码
        recall = 0
        precision = 0
        hmean = 0
